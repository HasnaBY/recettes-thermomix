...
export default function MenuPdfDownloadButton({
  ...
  const fetchRecipesAndAssets = async () => {
    ...
    const { data: logoData } = await supabase
      ...
      .in('key', ['round_logo', 'menu_pdf_background', 'shopping_list_pdf_background', 'client_menu_pdf_background'])

    const siteLogo = logoData?.find((l) => l.key === 'round_logo')?.image_url ?? null
    const adminBackgroundImage = logoData?.find((l) => l.key === 'menu_pdf_background')?.image_url ?? null
    const shoppingListBackground = logoData?.find((l) => l.key === 'shopping_list_pdf_background')?.image_url ?? null
    const clientMenuBackground = logoData?.find((l) => l.key === 'client_menu_pdf_background')?.image_url ?? null

    return { recipes, siteLogo, adminBackgroundImage, shoppingListBackground, clientMenuBackground }
  }
  ...
  const handleDownloadMenu = async () => {
    ...
    setError('')
    try {
      const { recipes, adminBackgroundImage, clientMenuBackground } = await fetchRecipesAndAssets()
